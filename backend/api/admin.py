...
class CustomUserAdmin(UserAdmin):
    # Đập đi xây lại cấu trúc form User
    fieldsets = (
...
# TÍNH NĂNG ĐÁNG GIÁ NHẤT: Xếp lớp bằng cách kéo thả 2 cột
@admin.register(ClassGroup)
class ClassGroupAdmin(admin.ModelAdmin):
    list_display = ('name', 'course', 'band', 'status')
    list_filter = ('status', 'course', 'band')
    search_fields = ('name',)
    # Hiển thị 2 khung chọn nhiều cho Giáo viên và Học viên
    filter_horizontal = ('teachers', 'students') 

# 3. Học Viên & Học Phí
@admin.register(Student)
class StudentAdmin(admin.ModelAdmin):
    list_display = ('full_name', 'phone', 'status', 'parent_name')
    list_filter = ('status',)
...
class GradeRecordAdmin(admin.ModelAdmin):
    ...
    list_filter = ('is_finalized', 'class_group')
    search_fields = ('student__full_name',)